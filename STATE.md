...
## Current build: **b58** (badge in `game_template.html`, search `based-cup bNN ✓`)
Last verified: sprites re-normalised on FACE WIDTH — faces now read consistently sized (verified by a 14-head render grid), head heights now vary by hairstyle (244–350px, correct), 0 cropped, okeyjak still uncropped, all 14 load 360×450; grabCap still constant 48, no gameplay touched; 0 tokens, 0 placeholders, harness passes.
