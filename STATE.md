...
## Current build: **b59** (badge in `game_template.html`, search `based-cup bNN ✓`)
Last verified: goal explosion + shake + flash + slow-mo + GOAL slam wired and verified in-browser (debris blast away from the scored goal, 3 objs + 40 sparks, synced via goalSeq); boot-boilerplate loading line removed; grabCap still constant 48; 0 tokens, 0 placeholders, harness passes.
