...
## Current build: **b57** (badge in `game_template.html`, search `based-cup bNN ✓`)
Last verified: sprite re-cut — all 14 in-game head heights now 74–75px (was 57–83px), 0 cropped, okeyjak full face restored, all 14 load 360×450; grabCap still constant 48, no gameplay code touched; 0 tokens, 0 placeholders, harness passes.
✅ **b57 = sprite head-size + crop fix (process_sprites.py only).** Heads rendered at different visual heights (hair-driven, 57–83px) and okeyjak's chin was cropped (3/4 jaw fooled the auto chin-detect). Switched `process_sprites.py` from cheek-WIDTH normalisation to **visible HEAD-HEIGHT normalisation** (hair-top→chin → `TARGET_HEAD_H=268`, ≈ old median so the group barely moved), centre by the head's own bbox + width-contain (no side clip), and a per-char chin override `CHIN_OVERRIDE={'okeyjak':0.88}`. Re-cut all 14, rebuilt. Gameplay/grabCap/stats untouched.
...
| Sprite heads uniform visual height + no face crop (okeyjak) | ✅ done | b57 |
