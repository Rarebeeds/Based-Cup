...
## Current build: **b60** (badge in `game_template.html`, search `based-cup bNN ✓`)
Last verified: goal slow-mo reworked — anticipates (ball crosses at ~30% via sim frame-pulldown, ratio 0.31), gated to 2nd-half/last-20s/pre-goal-margin≤1, FORCE toggle works; all cases sim-tested; explosion unchanged, match clock real-time, grabCap 48; 0 tokens, 0 placeholders, harness passes.
...
- Slow-mo tunables (near `goalLock`): `GOAL_SLOWMO_SCALE=0.30` (1.0=off), `GOAL_SLOWMO_MS=320`, `SLOWMO_ZONE=110`px, `SLOWMO_BALL_SPEED=6`px/f, `SLOWMO_LAST_SECS=20`, `SLOWMO_MARGIN=1`, `FORCE_GOAL_SLOWMO=false`.
