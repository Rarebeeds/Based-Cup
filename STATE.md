...
## Current build: **b56** (badge in `game_template.html`, search `based-cup bNN ✓`)
Last verified: pace tuning — on/off + char-to-char speed spread widened ~3.1×, verified via live `statDerived`; per-char stats untouched (all total 36); steal/glue/defense constants untouched; grabCap still constant 48; 0 tokens, 0 placeholders, harness passes. ⏳ awaiting user feel-test.
